    ...
    {   name: "AllSongs",
        customList : [
            ...
            },
            {
                ...
                artist : "Atif Aslam Sameer ft Pritam"
            },
            {
                name : "Girls Like You",
...
for (var listCounter = 0; listCounter < playlist_List[0].customList.length; listCounter++) {
    ...
    li.innerHTML ='<img id = "thumbnail" src="'+playlist_List[0].customList[listCounter].image+'" height="40em" width="40em" style="border-radius: 5px;">'+l_name+'';  
...
function createNewPlaylist(){
    ...
    newNode.innerHTML = '<h2><button class="playlist-btn" id=btn-'+ playlist_List[totalPlaylists].name +'" onclick="selectCustomlist(this.id)"><i class="fas fa-play"></i></button>' +  playlist_List[totalPlaylists].name + '</h2><ol id="'+playlist_List[totalPlaylists].name+'"></ol>'+' <button class="playlist-btn" id=rmv-'+ playlist_List[totalPlaylists].name +'" onclick="deleteLastSong(this.id)">Remove Last Song</button>' ;
    document.querySelector('.listContainer').appendChild(newNode);


    var newListName = document.createElement("li");
    newListName.innerHTML = playlist_List[totalPlaylists].name + '<button class="playlist-btn" id=btn-'+ playlist_List[totalPlaylists].name +'" onclick="selectCustomlist(this.id)"><i class="fas fa-play"></i></button>';
